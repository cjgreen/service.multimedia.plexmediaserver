    ...
    def ensure_agent_info_exists_inner():
      ...
      def info_lock():
        if identifier not in self.agent_info:
          ...
          HTTP.Request("http://127.0.0.1:32400/:/plugins/%s" % identifier, immediate=True, cacheTime=0)
  ...
  def update_task(self, mediaType, media_type, identifier, guid, id, lang, dbid, parentGUID, force, agent, async, parentID, libraryAgent=None, periodic=False):
    ...
    try:
      try:
        ...
        #
        if len(contributing_agents) == 0 and identifier == "com.plexapp.agents.localmedia":
